...
bids_type_summary %>%
  kable("html", escape = FALSE, align = c("l", rep("c", ncol(bids_type_summary) - 1))) %>%
  row_spec(which(!bids_type_summary$match), background = "#62ec0001") %>%
  column_spec(seq_len(ncol(bids_type_summary)), extra_css = "padding-right: 30px; padding-left: 20px;") %>% # nolint
...
# Download U.S. ZCTA polygons (2020)
...
# inspect suspicious ZIPs that were cleaned

# table(bids$DEVICE_GEO_ZIP_clean, useNA = "always")
...
To clean the ZIP codes in our Oregon-only dataset, we apply rules that match how ZIP codes work in the state and use the location data to fill in gaps. Any missing ZIP codes are filled by mapping each record’s latitude and longitude to the ZIP Code Tabulation Area it falls within. We then tidy the results by trimming extra spaces, removing placeholder values like and keeping only valid Oregon ZIP codes, which must be five digits and start with “97.” This gives us a clean and realistic ZIP field that matches the geography of the data with only 20 NAs remaining in the clean zip variable.
...
There is no set of cols that we believe can use used as a unique key. To remedy this, we added a row_id column to the bids dataframe to use as a unique key. See earlier work for why there is no good unique key in the original dataset
...
We are checking for duplicates across all columns except row_id, which we know is unique.
...
We will be dropping exact full-row duplicates from the dataset. These duplicates represent
records taht are identical across all the relevant variables and likely result from data collection
and/or ingestion errors.

We will be keeping only the first occurence of each unique row and removing subsequent duplicates. This will ensure that we retain one valid representation of each bid while eliminating redundancy that could distort summaries and analysis results.

No additional rules will be implemented.Removing duplicates based on content was sufficient to clean this aspect of the dataset.

> 5. Implement your rule and create a de-duplicated dataset (e.g., `bids_nodup`).
we are keeping the first occurance and dropping all other duplciated rows
...
We added a key to ensure that each row can be uniquely identified. For duplicate rows, we retained the first occurrence and removed all subsequent repeats. This approach preserves all available data while preventing over-counting of identical measurements.

However, because no combination of existing columns guarantees a truly unique identifier, there is a change, although small, that we delete a unique row. This could occur if two rows contain identical information but are in fact distinct. While this scenario is highly unlikely, it cannot be ruled out entirely.
...
```{r}
# find summary for lat and long

summary(bids[, c("DEVICE_GEO_LAT", "DEVICE_GEO_LONG")])
...
```{r}
# checking for coordinates outside the Oregon range
...
```{r}
# Define plausible Oregon bounding box
lat_min <- 42
lat_max <- 46.5
long_min <- -125
long_max <- -116
...
The latitude and longitude values were cleaned using simple geographic rules based on Oregon’s expected coordinate range. Latitudes between 42 and 46.5 degrees and longitudes between –125 and –116 were considered plausible and retained. Values outside these bounds were treated as implausible for Oregon-only data and set to NA in the cleaned variables. This approach ensures that realistic Oregon locations are preserved while clearly incorrect coordinates are flagged.

## Clear BID_WON (Option)
```{r}
...
>    - Uses your cleaned variables (`PRICE_final`, `DEVICE_GEO_REGION_clean`, `DEVICE_GEO_ZIP_clean`, `RESPONSE_TIME_clean`, `TIMESTAMP_clean`, `DEVICE_GEO_LAT_clean`, `DEVICE_GEO_LONG_clean`, etc.),
>    - Drops unnecessary intermediate columns (e.g., `_raw`, helper strings) that you don’t need for analysis.

```{r}
# Your code here
#since some of the data cleaning happened after I created the bid_nodup I need to adjust

# Recreate deduplicated version from latest bids
bids_nodup <- bids %>%
...
# View the result & verify all cleaned data made it in
glimpse(bids_clean)
...
| DEVICE_GEO_REGION | *Inconsistent region codes* | *Consistency* | *Standardized all Oregon variants to "OR" and set non-Oregon values to NA.* |
| DEVICE_GEO_ZIP | *Missing/Invalid ZIP codes* | *Completeness + Validity* | *Geocoded missing ZIPs from lat/long, filtered to Oregon-only ("97..." prefixes), trimmed spaces, removed placeholders.* |
| RESPONSE_TIME | *Includes non-numeric characters* | *Validity* | *Stripped the text labels and kept only the numbers using regex* |
| TIMESTAMP | *Inconsistent/incomplete TIMESTAMP formats* | *Completeness + Consistency* | *Combined DATE and TIMESTAMP columns into a single POSIX datetime column* |
| DEVICE_GEO_LAT(optional) | *Implausible coordinate values outside Oregon's geographic range.* | *Validity* | *Filtered Latitudes from 42 to 46.5, set outliers to NA.* |
| DEVICE_GEO_LONG(optional) | *Implausible coordinate values outside Oregon's geographic range.* | *Validity* | *Filtered Longitudes from –125 to –116, set outliers to NA.* |
| BID_WON(optional)) | *Inconsistent word of TRUE* | *Consistency* | *Standardized all "true""TRUE" to "TRUE"* |
...
*The task of clearing DEVICE_GEO_ZIP was the most time-consuming, because the step of download 2020(year) U.S. ZIP Code Tabulation Area(ZCTA) polygons and the step of using ZCTA to fill missing ZIP codes take us a total of 5 mins to run the code.*
*The task of clearing DEVICE_GEO_REGION was the most surprising, because about 15 thousand data entries use “xor” to represent Oregon, and we have no idea why the data collector did this.*
*‘mutate()’ + ‘case_when(), were most valuable for rule-based transformations,‘str_extract()’ was most valuable for text/number separation,‘parse_date_time()’ was most valuable for datetime standardization.*
*If we receive a new messy dataset including ZIP data, we will try to find a different way for clearing ZIP data so not to sit there and wait 5 mins to run a code.*
*We use generative AI to determine what code meets our requirements and how to fix errors when the code occurs, and determine which are the unnecessary intermediate columns that we can drop for the final cleaned dataset.*
