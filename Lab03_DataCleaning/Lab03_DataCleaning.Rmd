...
```{r}
# Your code here
#Neal left a note saying there was already code to check for duplicates, have him confirm and adjust comment here
...
#creating a de-duplicated dataset, minus 'row_id'
bids_nodup <- bids %>%
  distinct(across(-row_id), .keep_all = TRUE)

#Original rows: 443969, After de-duplication: 441535

```

> 6. In 2–3 sentences, explain your choice and connect it to the **uniqueness** dimension of data quality.

We identified and removed 2,434 exact duplicate rows from the dataset, reducing it from 443,969 to 441,535 records. These duplicates were identical across all relevant columns), indicating redundancy without any added information. Removing them improves the uniqueness dimension of data quality by ensuring that each record represents a distinct bid event, preventing duplicate entries from skewing analysis or inflating counts.
