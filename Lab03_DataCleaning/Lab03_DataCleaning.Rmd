...
convert_column <- function(df, col_name, target_type,
  ...
  if (is.null(new_col_name)) {
    new_col_name <- paste0(col_name, "_clean")
  }

  # Apply preprocessing if provided
  if (!is.null(preprocess_fn)) {
    if (verbose) cat("Applying preprocessing...\n")
    df[[new_col_name]] <- preprocess_fn(df[[col_name]])
  }

  # Perform conversion with error handling
  tryCatch({
    if (verbose) cat(glue("Converting {col_name}...\n"))
    df[[new_col_name]] <- converter(df[[col_name]], ...)

    if (verbose) {
      cat(glue("{new_col_name} is now: {class(df[[new_col_name]])[1]}\n\n"))
    }
  }, error = function(e) {
    log_error(glue("Failed to convert {new_col_name}: {e$message}"))
    stop(glue("Conversion failed for {new_col_name}: {e$message}"))
  })
