...
Most of the region codes in the dataset look correct, but about a quarter of all rows (104325) don’t follow the expected two-letter state format. There is a possiblity of misclassified, or inconsistent recording.
...
Suspicious regions code found in the dataset are "Or", "oregon", and "xor". They are not two-letter U.S. state code and therefore violates the format defined by the Prebid Data Dictionary. They are likely representing different names for Oregon
...
# table
kable(response_patterns, caption = "Response petterns")
