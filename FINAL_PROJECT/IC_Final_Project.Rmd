...
ggplot(bids_clean, aes(x = factor(PUBLISHER_ID), y = PRICE_final,
  ...
  labs(
    title = "Bid Distributions by Advertiser",
    x = "Advertiser",
    y = "Bid Amount"
  )
...
```{r}
# Rebuild clean binary column from the original logical one
...
# Create train (60%) and test (40%) indexes
n <- nrow(bids_clean)
train_index <- sample(seq_len(n), size = 0.6 * n)

# Split data
train_data <- bids_clean[train_index, ]
test_data  <- bids_clean[-train_index, ]

# Fit BAM model on training data

model <- bam(
  BID_WON_clean ~
    s(PRICE_final, k = 10) +
    te(DEVICE_GEO_LONG_clean, DEVICE_GEO_LAT_clean, k = c(8, 8)),
  data = train_data,
  family = binomial(),
  discrete = TRUE
)
...
ggplot() +
  ...
  labs(
    title = "Model-Predicted Win Probability Across Oregon Cities",
...
bids_clean <- bids_clean %>%
  mutate(
    ...
    SIZE = as.factor(SIZE),
    AUCTION_ID = as.factor(AUCTION_ID)
  )
...
```
```{r}
# 1.2 Distribution of winning bids vs losing bids with custom colors
ggplot(bids_clean %>% filter(!is.na(PRICE_final)),
  ...
  labs(
    title = "Distribution of Bid Prices by Win Status",
    subtitle = "Logarithmic scale for better visualization",
...
```
```{r}
# 1.3 Response time analysis by win status with custom colors
ggplot(bids_clean, aes(x = RESPONSE_TIME_clean, fill = BID_WON_clean)) +
  ...
  labs(
    title = "Distribution of Response Times by Win Status",
    x = "Response Time",
    y = "Density"
  ) +
  theme_minimal() +
