...
> CHANGED THIS TO Geographic Analysis:
  * Visual reprsentation of the data in a geographic map
  * Q: how does the proximity to portland affect the price of a bid?

The data that needed to be cleaned was not a significant portion of the total data set, so there was not a significant change
...
```{r}
population <- read_csv(here("data", "2025 Preliminary Population Estimates.csv"))
population <- population %>%
  rename(city = `Incorporated City/Town`,
         population = `Revised Population Estimate july 1, 2024`)
# Get unique cities from hex_with_city
hex_cities <- unique(hex_with_city$major_city)

# Check what column has city names in population (might need to adjust)
names(population)

# Find cities in hex_with_city that are NOT in population
# (adjust "city_column" to the actual column name in population)
missing_cities <- setdiff(hex_cities, population$city_column)
