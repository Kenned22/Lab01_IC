...
### **4. Geographic Analysis**
  ...
  * Is there a relationship between bid pricing, all and winning bids, and proximity to portland (major city)
  * Is there there a meaningful difference in the bid behavior of rural (population < 50,000) vs urban (pop > 50,000)


Original question looked at impacts of data cleaning on analysis. The data that needed to be cleaned was not a significant portion of the total data set, so there was not a significant change
in staticsis of the data before vs after cleaning.
...
ggplot(feature_importance_summary, aes(x = reorder(Feature, Value), y = Value, fill = Importance_Level)) +
  ...
  geom_text(aes(label = round(Value, 3)),
            hjust = -0.1, size = 3.5) +
  coord_flip() +
  scale_fill_manual(values = c("High" = "#FFE945", 
                             "Medium" = "#7C7B78", 
                             "Low" = "#00204D")) +
  labs(
...
hb <- hexbin(
  ...
  xbins = 15,  # same as bins in ggplot
  IDs = TRUE
)
...
ggplot() +
  ...
  labs(
    title = "Average Bid Price by Region",
    subtitle = "Oregon",
    fill = "Avg Price",
    x = NULL,
    y = NULL
...
This plot would not be possible without properly cleaning the lat/long data. Having consistent, valid lat/long data allowed us to use other packages and data sets (zip code data) to make plots like this and perform geographic analysis. If we were not confident in the geographic data of the cleaned data set, we would not be able to perform this or the following two analyses. 

This is more for a visual perspective into bid price distribution. Did not want to draw any major conclusion from this graph. 
...
Cleaning the zip code data allowed us to confidently segment oregon into 60 hexes and bin the data into the appropriate hex. With the binned data, we could then perform the above analysis to explore if there was any correlation between distance to portland and the average winning bid price. 

Plotted average winning bid price, by hex, vs  the distance the hex center was from portland. There is a weak, or no, relationship between winning bid price and distance to portland. This may point to there being no correlation between geography and winning bid price, or the indicator is more complex than just distance to portland. 

#
Urban vs Rural comparison

compared average price, median price, price standard deviation, win rate, and average response time for area with populations above 50,000 (urban) and those below 50,000 (rural)
```{r}

# Top populated urban areas (> 50,000) that are present in bids_clean$city
urban_cities <- c("Portland",
                  "Salem",
                  "Eugene",
                  "Gresham",
                  "Hillsboro",
...
urban_summary <- bids_clean %>%
  ...
  summarise(
    n_bids = n(),
    avg_price = mean(PRICE_final, na.rm = TRUE),
    avg_price_winning = mean(PRICE_final[BID_WON_clean == TRUE], na.rm = TRUE),
...
urban_summary %>%
  pivot_longer(cols = c(avg_price, avg_price_winning, sd_price, median_price, win_rate, avg_response_time),
               names_to = "metric", values_to = "value") %>%
  ggplot(aes(x = area_type, y = value, fill = area_type)) +
  scale_fill_viridis_d(option = "cividis") +
  geom_col() +
  facet_wrap(~metric, scales = "free_y", nrow = 2, labeller = label_wrap_gen(width = 5)) +
  labs(title = "Urban (pop > 50,000) vs Rural Comparison (pop < 50,000)", x = "", caption = "85373 rural bids, 355610 urban bids") +
  theme_minimal() +
  scale_fill_viridis_d(option = "cividis") +
  theme(
    legend.position = "none",
    strip.text = element_text(size = 8)  # adjust size
  )



```
Without proper zip code and city data, we would not have been able to segment the bid data into rural and urban groups. By ensuring that all city data was accurate and matched the cleaned zip data, we could confidently group the data and perform the above analysis. 

We see higher prices and win rates for rural bids compared to urban ones. The higher bids and win rates may point to companies and individuals placing ads in more rural areas are willing to pay more, and win more, based on the average winning price and win rate. 
